...
async function changeUsername(newUsername, userid, req) {
    let login = (jwtService.isLoggedIn(req));
    ...
    if (user.constructor.name == 'ReturnMessage') {
        return user;
    }

    if (userid != login.sub && userService.admin(login.sub) == false) {
        return new ReturnMessage("2011", "Access Denied", 403, 'error');
    }
    ...
    let change = (await userService.changeUsername(userid, htmlED.encode(newUsername)))
    if (change !== true) {
        return change;
    }
...
async function changeEmail(newEmail, currentPassword, userid, req) {
    let login = (jwtService.isLoggedIn(req));
    ...
    let isAdmin = (userService.admin(login.sub))
    if (userid != login.sub && !isAdmin) {
        return new ReturnMessage("2350", "Access Denied", 403, 'error')
    }
    ...
    if (userid == login.sub && !isAdmin) {
        ...
        if (currentPassword == undefined || !(await userService.verifyPass(myData.pass, currentPassword))) {
            return new ReturnMessage("2306", "Bad Password", 400, "error");
        }
    }

    if (user.email == newEmail) {
        return new ReturnMessage("2307", "New email cannot be the same as current one", 400, "error");
    }
